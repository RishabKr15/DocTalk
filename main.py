...
def install_ollama():
    ...
    try:
        ...
        if model_name in result.stdout:
            ...
            try:
                ...
                llm.invoke("test")
                print(f"Model {model_name} is accessible in CPU mode.")
                return model_name
            except Exception as e:
                print(f"Error accessing model {model_name}: {e}")
                print("Removing potentially corrupted model and retrying...")
    ...
    try:
        ...
        print(f"Error pulling or accessing model {model_name}: {e}")
        ...
        for alt_model in alternative_models:
            try:
                ...
                llm = OllamaLLM(model=alt_model, num_gpu=0, base_url="http://127.0.0.1:11434")
                llm.invoke("test")
                print(f"Successfully accessed model: {alt_model}")
                return alt_model
            except Exception:
                continue
        
        print("Could not pull or access any model. Please check your Ollama installation.")
        sys.exit(1)
...
def initialize_rag_chain(vector_store, model_name='llama3.2:1b'):
    ...
    answer_prompt = PromptTemplate.from_template(
        """Using the following context from the provided documents, answer the question accurately and concisely. If the context does not contain enough information to answer the question, state that clearly. Do not generate information beyond what is provided in the context.
